...
public class CommandHandler {
	...
	/**
		*处理单个命令并返回字符串输出。每条命令开始
		*带有时间戳和命令（例如，查找，插入，删除），后面跟着
		*参数列表。参数以冒号（":"）分隔。例如,
		*删除LAX机场的命令为“delete:LAX”。(注意时间戳
		*在此版本中被忽略。)
		*注意，插入和删除命令都首先执行查找以避免
		*产生异常。
		*preorder命令在预订中获得机场列表。它打印
		*此列表，并生成一个很好的缩进（按顺序）表示
		*树的内容。
		＊
		* @param inputLine 命令和参数的输入行。
		* @return 命令执行/结果的简短摘要。
	*/
