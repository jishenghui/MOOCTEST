...
//		ControlPanel controlPanel = new ControlPanel(new AlgAnimFrame(new AlgAnimApp(), new URL("HTTP","8080","src/test/resources/test.txt")), "test.txt");
		AlgAnimApp algAnimApp = new AlgAnimApp();
//		algAnimApp.
//		algAnimApp.fn_labesl = "src/test/resources/filename.txt";
	
		URL url = new URL("http://localhost:8080?filename=filename.txt");
		algAnimApp.homeURL = new URL(url.getProtocol(), url.getHost(), url.getPort(),url.getPath());
		algAnimApp.sourceURL = new URL("HTTP", "8080", "src/test/resources/filename1.txt");
		algAnimApp.
		algAnimApp.init();
//		algAnimApp.homeURL;
		Event event = new Event(new Object(), 1, new Object());
//		event.target = null;
		algAnimApp.action(event, new Object());
...
//		heap.input2heap();
	...
	public void test4() throws Exception {
		URL url=new URL("http://127.0.0.1/index.html?algname=11");
		AlgAnimApp algAnimApp = new AlgAnimApp();
		AlgAnimFrame frame = new AlgAnimFrame(algAnimApp, url);
		AlgThread algThread = new AlgThread(frame);
		algAnimApp.start();
...
//		lFrame.handleEvent(new Event(new Object(), 1, new Object()));
