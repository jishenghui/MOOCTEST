...
回溯
DFS
给定一个整数 n，求解所有不同的 N 皇后问题的解决方案。N 皇后问题是指在 n x n 的棋盘上放置 n 个皇后，使得它们不能相互攻击（即同一行、同一列和同一对角线上不能有两个皇后）。

输入格式
一个整数 n，表示棋盘的大小。

输出格式
对于每个方案，输出一个 n x n 的棋盘，其中 'Q' 表示一个皇后，'.' 表示一个空位。输出方案时，每个方案占用 n 行，每行输出一个长度为 n 的字符串表示当前行的棋盘布局。方案之间以空行分隔。
